...
serve(async (req) => {
  ...
  try {
    ...
    const { type, startDate, endDate, page = 1 } = await req.json();
    ...
    if (type === 'nfe') {
      ...
      const response = await fetch(`${OMIE_API_URL}/produtos/nfconsultar/`, {
        ...
        body: JSON.stringify({
          ...
          param: [{
            ...
            ordenar_por: 'CODIGO',
            ...(startDate && { dEmiDe: startDate }),
            ...(endDate && { dEmiAte: endDate }),
          }],
        }),
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(`Erro Omie NFe [${response.status}]: ${errorText}`);
      }
      ...
      const response = await fetch(`${OMIE_API_URL}/produtos/cupomfiscalconsultar/`, {
        ...
        body: JSON.stringify({
          ...
          param: [{
            nPagina: page,
            nRegPorPagina: 50,
            ...(startDate && { dDtEmissaoDe: startDate }),
            ...(endDate && { dDtEmissaoAte: endDate }),
          }],
        }),
      });
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(`Erro Omie NFCe [${response.status}]: ${errorText}`);
      }
      ...
      result = {
        ...
        invoices: (data.cupons || []).map((cupom: any) => ({
          id: cupom.cabecalhoCupom?.nIdCupom,
